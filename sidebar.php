    ...
    <nav class="menu">
        ...
        <?php if ($current_role === 'admin'): ?>
            ...
            <a href="users_manage.php" class="menu-item <?= $current_page === 'users_manage.php' ? 'active' : '' ?>" style="display: flex; justify-content: space-between; align-items: center;">
                ...
                <?php
                // Получаем количество пользователей для бейджа
                $stmtCount = $pdo->query("SELECT COUNT(*) FROM users");
                $totalUsers = $stmtCount->fetchColumn();
                ?>
                <span style="background: var(--accent); color: white; font-size: 0.75rem; font-weight: 700; padding: 0.1rem 0.45rem; border-radius: 999px; line-height: 1.6; min-width: 20px; text-align: center;"><?= $totalUsers ?></span>
            </a>
        <?php endif; ?>
